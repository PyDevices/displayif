...
// Pin(id|name|Pin, mode, value=value) — for ports where pin ids are strings (mimxrt).
...
// SoftSPI(baudrate=..., polarity=..., phase=..., sck=..., mosi=..., miso=...).
mp_obj_t displayif_machine_softspi(mp_obj_t kwargs);
// machine.SoftSPI(**kwargs) — keyword-only (no bus id).
mp_obj_t displayif_machine_softspi(mp_obj_t kwargs);
