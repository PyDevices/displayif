...
//     the image slides horizontally under load.
//   * Double panel FBs + auto_refresh=False — tear-free present for LVGL;
//     fill_rect/blit without show()/refresh() leaves the panel on the old
//     front (looks black/white). A single-FB + auto_refresh=True experiment
//     made fill_rect visible without show() but brought mid-scan blackouts
//     back under LVGL animation (T-RGB / LCD-7).
//
