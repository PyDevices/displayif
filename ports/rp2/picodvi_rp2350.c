...
void picodvi_rp2350_construct(picodvi_framebuffer_obj_t *self,
    ...
    for (size_t i = 0; i < 8; i++) {
        ...
        if ((all_allocated & mask) != 0) {
            mp_raise_ValueError(MP_ERROR_TEXT("duplicate HSTX pin"));
        }
        all_allocated |= mask;
    }
    ...
    // streaming support.
    self->framebuffer = (uint32_t *)m_malloc(framebuffer_size * sizeof(uint32_t));
    ...
    self->dma_commands = (uint32_t *)m_malloc(self->dma_commands_len * sizeof(uint32_t));
    if (self->dma_commands == NULL) {
        picodvi_rp2350_deinit(self);
        mp_raise_msg(&mp_type_MemoryError, MP_ERROR_TEXT("picodvi DMA command allocation failed"));
    }
...
void picodvi_rp2350_deinit(picodvi_framebuffer_obj_t *self) {
    if (picodvi_rp2350_deinited(self)) {
        return;
    }
